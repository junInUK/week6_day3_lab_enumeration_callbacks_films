...
describe('Cinema', function () {
  ...
  it('should be able to check whether there are some films from a particular year',function(){
    const actual = cinema.isFilmsFromYear(2017);
    assert.strictEqual(actual,true);
  });

  it('should be able to check whether there are no films from a particular year',function(){
    const actual = cinema.isFilmsFromYear(1999);
    assert.strictEqual(actual,false);
  });
  it('should be able to check whether all films are over a particular length',function(){
    const actual = cinema.areAllFilmsOverLength(90);
    assert.strictEqual(actual,true);
  });
  it('should be able to check whether all films are over a particular length (false)',function(){
    const actual = cinema.areAllFilmsOverLength(100);
    assert.strictEqual(actual,false);
  });
  ...
  it('should be able to filter films by year',function(){
    ...
    assert.deepEqual(actual,[bladeRunner,dunkirk,trainspotting]);
  })
